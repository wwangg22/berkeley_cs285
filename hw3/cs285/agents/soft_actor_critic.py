...
class SoftActorCritic(nn.Module):
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            # print("mean of qs ", torch.mean(next_qs))
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            q_values = torch.zeros(
                self.num_critic_networks, self.num_actor_samples, batch_size, device=action.device)
            for a in range(self.num_actor_samples):
                q_values[:,a,:] = self.critic(obs, action[a,:,:])
        ...
        log_probs = action_distribution.log_prob(action).sum(dim=-1, keepdim=True)  # Log-probs summed over action_dim
        weighted_log_prob = log_probs * advantage.unsqueeze(-1)
    ...
    def update_actor(self, obs: torch.Tensor):
        ...
        #         param_norm = p.grad.data.norm(2)  # Compute L2 norm of gradients for this parameter
        #         total_norm += param_norm.item() ** 2  # Accumulate squared norms

        # total_norm = total_norm ** 0.5  # Compute the total L2 norm
    ...
    def update(
        ...
        for a in range(self.num_critic_updates):
        ...
        if self.target_update_period != None and step % self.target_update_period==0:
            self.soft_update_target_critic(1.0)
        elif self.soft_target_update_rate != None and step % self.soft_target_update_rate == 0:
            self.soft_update_target_critic(0.01)
